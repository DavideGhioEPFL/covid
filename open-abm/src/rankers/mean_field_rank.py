...
def ranking_backtrack(t, transmissions, observations, delta, tau, mu, rng, delay=0):
    ...
    if(delay > 0):
        observations = [test for test in observations if test["t_test"] < t - delay]
...
class MeanFieldRanker(AbstractRanker):
    ...
    def __init__(self, tau, delta, mu, lamb, delay=0):
    ...
    def rank(self, t_day, daily_contacts, daily_obs, data):
        ...
        scores = ranking_backtrack(
            ...
            self.delta, self.tau, self.mu, self.rng, delay=self.delay
        )
